...
LOG="${LOG:-logs/${DOMAIN_KEY}_full_workflow_orchestrator.log}"
...
SUMMARY="${OUTPUT_ROOT}/final_panels/${DOMAIN_KEY}_policy_summary.json"
...
log_msg "$DOMAIN_KEY workflow orchestrator started; total=$TOTAL"
...
log_msg "Building $DOMAIN_KEY panels."
DOMAIN_KEY="$DOMAIN_KEY" PIPELINE_SCRIPT="$PIPELINE_SCRIPT" OUTPUT_ROOT="$OUTPUT_ROOT" \
  bash outputs/cloud_runbooks/build_domain_policy_panels.sh >> "$LOG" 2>&1

if [[ -s "$MERGED_WITH_TOOLS" && -s "$SUMMARY" ]]; then
  log_msg "$DOMAIN_KEY workflow complete."
  date -u '+%Y-%m-%d %H:%M:%S UTC' > "$COMPLETE_FLAG"
else
  log_msg "ERROR: panel build did not create expected outputs."
  exit 3
fi
