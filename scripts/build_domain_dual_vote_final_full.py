#!/usr/bin/env python3
"""Build full domain industrial-policy classification after boundary voting."""
...
def safe_name(value: str, max_len: int = 55) -> str:
...
def main() -> None:
    ...
    parser.add_argument("--prefix", default="domain_dual_vote_final")
    ...
    final_field = f"final_is_{args.domain_key}_industrial_policy"
            ...
            row.update(
                {
                    ...
                    "domain_label": args.domain_label or args.domain_key,
                    "final_vote_source": "dual_vote_boundary",
                    "dual_vote_final": label,
                    final_field: True if label == "yes" else False if label == "no" else None,
                    "requires_review": label == "split",
                    "dual_vote": merged,
                }
            )
        else:
            vote = policy_vote(stage1, args.domain_key)
    ...
    counts["domain_key"] = args.domain_key
