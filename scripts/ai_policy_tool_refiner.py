#!/usr/bin/env python3
"""
Refine policy-tool classifications for confirmed AI industrial policies.

The input should already contain confirmed artificial-intelligence industrial
policies. This script classifies tools, timing, side, tone, specificity,
strength, coverage, and concise evidence with one model.
"""
...
TOOL_SYSTEM_PROMPT = """你是研究中国人工智能产业政策的政策工具编码员。
只做政策工具和政策属性分类，不重新判断该文件是否属于人工智能产业政策。
必须严格依据标题和正文证据，输出且只输出一个 JSON 对象。
"""


TOOL_PROMPT_TEMPLATE = """请对以下“已确认属于人工智能产业政策”的文件进行具体政策工具分类。

分类原则：
1. 不再重新判断是否为产业政策；默认它已经是人工智能产业政策。
...
人工智能产业链环节可包括：基础研究、大模型/基础模型、生成式人工智能、算法模型、智能算力/智算中心、AI芯片、数据标注/语料、模型训练/推理、平台服务、场景应用、治理监管、人才、投融资、产业集群。
...
{{
  ...
  "target_segments": ["大模型|算法模型|智能算力|AI芯片|数据标注|场景应用|治理监管|人才|产业生态|其他"],
...
def is_confirmed_policy(row: Dict[str, Any]) -> bool:
    if row.get("final_is_ai_industrial_policy") is True:
        return True
        ...
        while True:
            ...
            for fut in done:
                ...
                if processed % max(1, args.progress_every) == 0 or processed == len(rows):
                    ...
                    print(
                        f"[AI TOOL REFINE] processed={processed:,}/{len(rows):,} "
