#!/usr/bin/env python3
"""
Refine concrete policy-tool classifications for confirmed NEV industrial policies.

This script is intentionally narrower than nev_policy_pipeline.py classify:
it assumes the input row is already a confirmed NEV industrial policy and asks a
single model to classify policy tools, timing, side, tone, specificity, strength,
coverage, and concise evidence. It is resume-safe JSONL output.
"""
...
from nev_policy_pipeline import (  # noqa: E402
    TOOL_DEFS,
    call_ollama,
    canonical_list,
    safe_float,
...
TOOL_SYSTEM_PROMPT = """你是研究中国新能源汽车产业政策的政策工具编码员。
只做政策工具和政策属性分类，不重新判断该文件是否属于新能源汽车产业政策。
必须严格依据标题和正文证据，输出且只输出一个 JSON 对象。
"""


TOOL_PROMPT_TEMPLATE = """请对以下“已确认属于新能源汽车产业政策”的文件进行具体政策工具分类。

分类原则：
1. 不再重新判断是否为产业政策；默认它已经是新能源汽车产业政策。
...
{{
  ...
  "target_segments": ["整车|电池|充电基础设施|零部件|消费端|公共部门|回收利用|其他"],
  "specific_measures": ["简短列出具体措施；没有则空数组"],
  "eligibility_conditions": ["补贴/准入/申报/采购/试点等条件；没有则空数组"],
...
def tool_table() -> str:
    ...
    for tool_id, meta in TOOL_DEFS.items():
        lines.append(f"- {tool_id}: {meta['label']}；group={meta['group']}")
    return "\n".join(lines)
...
def normalize_tool_result(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    specificity = norm_choice(raw.get("measure_specificity"), VALID_SPECIFICITY, "guidance_only")
    tone = norm_choice(raw.get("policy_tone"), VALID_TONE, "support")
    timing = norm_choice(raw.get("timing"), VALID_TIMING, "ex_ante")
    side = norm_choice(raw.get("policy_side"), VALID_SIDE, "ecosystem")
    confidence = safe_float(raw.get("tool_confidence"), 0.0)
    if confidence < 0:
        confidence = 0.0
    if confidence > 1:
        confidence = 1.0
...
def prompt_for_row(row: Dict[str, Any], max_body_chars: int) -> str:
...
def is_confirmed_policy(row: Dict[str, Any]) -> bool:
...
def refine_one(row: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
    prompt = prompt_for_row(row, args.max_body_chars)
    try:
        raw, _ = call_ollama(
            prompt=prompt,
            model=args.model,
...
def main() -> None:
    ...
    for row in iter_jsonl(input_path):
        if str(row.get("id")) in completed:
            continue
        if not args.include_unconfirmed and not is_confirmed_policy(row):
            continue
        rows.append(row)
        ...
        while True:
            ...
            for fut in done:
                ...
                if processed % max(1, args.progress_every) == 0 or processed == len(rows):
                    ...
                    print(
                        f"[TOOL REFINE] processed={processed:,}/{len(rows):,} "
